#!/usr/bin/env groovy
// =============================================================================
// TicTacToeApp — Jenkins CI/CD Pipeline for Kubernetes Agents
// =============================================================================
// Builds a static web app, packages with Nginx,
...
pipeline {
    agent {
        kubernetes {
            label 'tictactoeapp-kaniko-agent'
...
'''
    ...
    stages {
        ...
        // -----------------------------------------------------------------------
        // Stage 4: Static Analysis — validate HTML, check file structure
        // -----------------------------------------------------------------------
        stage('Static Analysis') {
            steps {
                container('tools') {
                    sh '''
                        set -eux
                        apk add --no-cache htmlhint >/dev/null 2>&1 || true
                        echo "✅ File structure check"
                        test -f index.html      && echo "  index.html    — present"
                        test -f css/style.css   && echo "  css/style.css — present"
                        test -f js/audio.js     && echo "  js/audio.js   — present"
                        test -f js/game.js      && echo "  js/game.js    — present"
                        test -f js/app.js       && echo "  js/app.js     — present"
...
EOF
        ...
        // -----------------------------------------------------------------------
        // Stage 6: Build & Push Docker Image (Kaniko)
        // -----------------------------------------------------------------------
